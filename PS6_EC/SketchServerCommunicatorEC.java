 ...
 */
public class SketchServerCommunicatorEC extends Thread {
	private Socket sock;					// to talk with client
	private BufferedReader in;				// from client
	private PrintWriter out;				// to client
	private SketchServerEC server;			// handling communication for

	public SketchServerCommunicatorEC(Socket sock, SketchServerEC server) {
		this.sock = sock;
		this.server = server;
	}
	...
	private void processCommand(String cmd) {
		...
		switch (tokens[0]) {
			...
			case "f" -> {		// brings shape in front of all shapes
				server.getSketch().bringShapeToTop(Integer.parseInt(tokens[1]));
				server.broadcast(cmd);
			}
			case "b" -> {		// sends shape behidn all shapes
				server.getSketch().pushShapeBack(Integer.parseInt(tokens[1]));
				server.broadcast(cmd);
			}
		}
	}
	...
	private int addShapeToSketch(String[] tokens) {
		SketchEC serverSketch = server.getSketch();
		ShapeEC shape = null;
		switch (tokens[1]) {
			case "ellipse":
				shape = new EllipseEC(Integer.parseInt(tokens[2]),
				...
				break;
			case "rectangle":
				shape = new RectangleEC(Integer.parseInt(tokens[2]),
												Integer.parseInt(tokens[3]),
												Integer.parseInt(tokens[4]),
												Integer.parseInt(tokens[5]),
												new Color(Integer.parseInt(tokens[6])));
			...
			case "freehand":
				...
				shape = new PolylineEC(new Point(Integer.parseInt(tokens[2]), Integer.parseInt(tokens[3])),
				...
				break;
			case "segment":
				shape = new SegmentEC(Integer.parseInt(tokens[2]),
											Integer.parseInt(tokens[3]),
											Integer.parseInt(tokens[4]),
											Integer.parseInt(tokens[5]),
											new Color(Integer.parseInt(tokens[6])));
